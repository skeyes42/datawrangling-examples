...
team_summary <- df            |>
  group_by(TEAM_ID)           |>
    mutate(
      ...
      slice(1) |>
        ungroup()             |>
          select(TEAM_ID, avg_fgm, total_fgm, n_players)
...
game_summary <- df            |>
  group_by(GAME_ID)           |>
    mutate(
      ...
      slice(1)                |>
        ungroup()             |>
          select(GAME_ID, avg_fgm, total_fgm)
...
game_team_summary <- df       |>
  group_by(GAME_ID, TEAM_ID)  |>
    mutate(
      ...
      slice(1)                |>
        ungroup()             |>
          select(GAME_ID, TEAM_ID, avg_fgm, total_fgm)

print("Summary by game and team:")
print(game_team_summary)

"""
Claudes comments in the context of its R equivalent:
