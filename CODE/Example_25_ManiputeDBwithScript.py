...
def flush_database(path_to_scripts):
    ...
    try:
        subprocess.run(["sqlite3", db_path], input=drop_command.encode('utf-8'), check=True, cwd=path_to_scripts)
    ...
    try:
        subprocess.run(["sqlite3", db_path], input=recreate_command.encode('utf-8'), check=True, cwd=path_to_scripts)
    except subprocess.CalledProcessError as e:
...
def db_list_tables(con):
    """
    Equivalent to R's dbListTables using a SQL query.
    """
    cursor = con.cursor()
...
if not path_to_scripts or not os.path.isdir(path_to_scripts):
    print("Environment variable 'EXAMPLES' is not set or directory does not exist.")
    exit()
...
try:
    subprocess.run(["sqlite3", path_to_database], input=drop_command.encode('utf-8'), check=True, cwd=path_to_scripts)
except subprocess.CalledProcessError as e:
...
try:
    subprocess.run(["sqlite3", path_to_database], input=recreate_command.encode('utf-8'), check=True, cwd=path_to_scripts)
except subprocess.CalledProcessError as e:
